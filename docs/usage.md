...
Reproduce the native local D1 query plans, page sizes, equal-time keyset walk, and sparse scan receipt with `pnpm --filter web perf:usage --size=100000`. Use `--size=120001` for the unique priced match beyond 100,000 candidates. The script creates and replaces only `apps/web/.wrangler-usage-scale`, applies the shipped migrations, and prints a JSON receipt; it never addresses a remote database or inherits `TINES_API_URL`. Its wall time includes Wrangler startup and its memory figure is the documented conservative typed-sample bound rather than an isolate-inspector measurement.
