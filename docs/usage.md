...
On an issue page, the owner's **Agent activity** card shows a compact **Total spend** line from that direct lifetime aggregate, including attempts beyond the latest 20 activity rows. It labels partial and pending cost, retains the previous cutoff during a refresh, and offers Retry if the initial report fails. Members see the operational activity without this account-owned total. The page does not expose a signed evidence scope; use the API, CLI or Agents Spend views for contributing-run evidence.
