...
- Project and issue reads resolve the actual actor. A run key cannot use its user's memberships as human authority. An inaccessible target returns 404. Members can comment, repair their own human comments, take current awaiting-human transitions, and change only their own personal permission in a browser session. The owner can invite, resend, cancel, remove and moderate comments.
...
Member activity and issue history use an explicit type allowlist in the database query before the page limit. Payloads include only type-specific display fields and current issue/project references, with the original actor's name. Only `context.*` events whose kind is `artifact` enter that feed. Unknown types, private context, prompt, journal, account, run detail, credential, routing and usage data are outside the member projection. A removal tombstones the membership revision, clears that person's choices, revokes their project run keys and records cancellation intent for their active project work. A read still in flight checks the revision again and discards its response if removal wins.

Member comments, awaiting-human decisions and personal browser permission are now enabled. First opt-in shows the full evolving-work warning before its acknowledgement is saved with a successful choice; later controls show a short reminder. Member execution remains unavailable in this release.
