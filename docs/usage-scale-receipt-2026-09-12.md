...
The script replaces only `apps/web/.wrangler-usage-scale`, applies shipped migrations, never selects a remote database, and prints the complete JSON receipt including EXPLAIN rows. Unpaginated groups/rate identities remain proportional to their historical cardinality and are not silently capped.
