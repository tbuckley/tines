<script lang="ts">
	...
	/**
	 * The title line is one line at every width: the title truncates and every
	 * marker after it holds its size. Labels never sit on that line on a phone —
	 * a name there would leave the title unreadable — they get a line of their
	 * own below it, which `LabelStrip` fits to exactly one line. From `sm` up
	 * they stay inline, capped at three chips and a "+N". Both variants render
	 * and CSS picks, so there is no matchMedia and no hydration mismatch. The
	 * full set always lives on the detail page.
	 */
	const MAX_CHIPS = 3;
		...
		{#each issues as issue (issue.id)}
			<li animate:flip={{ duration: dur() }} in:fade={{ duration: dur() }}>
				...
				>
					...
					<!-- One line per row on a wide screen; on a phone the labels drop to
					     a second line of their own, which the strip keeps to one line. -->
					<span class="flex min-w-0 flex-1 flex-col gap-1 overflow-hidden text-sm font-medium">
						<!-- The title is the only thing that shrinks; every marker after it
						     is `shrink-0`, so nothing here can wrap. -->
						...
						{#if issue.labels.length > 0}
							<LabelStrip labels={issue.labels} class="sm:hidden" />
						{/if}
					</span>
					{#if showProject}
