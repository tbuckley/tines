 ...
 * delivery, per environment: the `local` variant ships now; the Claude and
 * Gemini variants slot in with their milestones (their differences are the
 * auth story and the bootstrap section).
 *
...
export type PreambleVariant = 'local'; // 'claude_managed' | 'gemini_managed' arrive in M2/M4
...
const VARIANTS: Record<PreambleVariant, { auth: string[]; workspace: string[] }> = {
	local: {
		auth: [
