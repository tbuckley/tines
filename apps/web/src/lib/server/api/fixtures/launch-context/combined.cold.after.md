...
Older agent comments: cmt_old_detail, cmt_old_test, cmt_old_docs, cmt_old_review, cmt_old_followup. Load one (change --arg id to a listed ID): `tines issues show 'Fixture Project/520' --json | jq -er --arg id cmt_old_detail 'first(.comments[] | select(.id == $id) | .body) // error("comment not found: \($id)")'`. Without jq / for full history: `tines issues show 'Fixture Project/520'`.
