...
export const load: PageServerLoad = async ({ locals, platform, params }) => {
	...
	// Only the issue itself and its event feed block the page render (the feed
	// also drives the live-update poll). The sidebar panels — issue context,
	// effective context, and agent activity — fetch client-side after
	// hydration instead (see $lib/deferred.svelte.ts); projects/workflows come
	// from the (app) layout load.
