...
Both `/issues` and `/projects/:id` render Previous and Next links. Filters remain in the URL and any filter change clears page boundaries. This includes `workflow` and `state`; changing or clearing workflow also clears its dependent state in the same navigation. The global issues page includes a comparison-only `page_scope` marker so a changed project focus resets a bounded URL to its first page. Project pages derive scope from the path.
