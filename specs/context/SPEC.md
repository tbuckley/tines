...
*Surfaces (Tines/269).* Each state in the workflow editor has an **Inherits from** picker listing every state the operator can see, grouped by workflow with base-like workflows first and the state's own workflow next (intra-workflow pointers are the API's primary resolution path); only the state itself is omitted, and "None" clears. An existing row emits `inherits_from` **only when the operator changed it**, so the merge-patch guarantee above stays structural rather than test-enforced. Below the picker sits a collapsed, read-only preview of the base's `instructions`, the base's other children, and an "Edit base" link. Cycle, depth and live-base ("this base's deltas reach N issues") checks are **advisory warnings** computed from the workflow library the page already has: the API remains the authority, and its refusal message (`inheritance_cycle`, `inheritance_too_deep`, `inheritance_target_removed`) renders verbatim in the editor's error banner. Removing a state or workflow that others inherit from offers `force_clear_inheritance` through a confirm dialog listing the children.

The derivations behind all of this — the library index, `inherits from` / `inherited by`, base-likeness, chain depth — live in `@tines/shared`'s `state-library.ts`, which `tines workflows show` / `bases` and the web surfaces both read, so the CLI and the editor agree by construction.
...
- **Issue detail** gains a **Context** panel with two parts:
  ...
  - *Revised 2026-09 (Tines/269):* every part, skill, repo and overridden entry that matched through an ancestor of the issue's state carries, after its scope badge, a dashed `via <workflow> / <state>` chip linking to the base's state on its workflow page, plus `also inherited by …` naming the base's other children. A part with no `inherited_from` renders exactly as before.
...
- **Project detail** gains a **Context** section: project-only items first, then `project ∧ state` items grouped under their state names (issue-anchored items excluded per the rule above). Inline create defaults the scope to this project, with an optional "only in state…" refinement.
  - *Revised 2026-09 (Tines/146):* the `project ∧ state` items are collapsed into one accordion row per workflow (closed by default, single-select, workflows alphabetical, states in workflow position order, one `{n} states · {m} items` chip) beneath the still-expanded project-only list, and the whole section moves below Issues, Scheduled tasks and Agent routing — 17 one-row state groups had pushed the Issues heading ~1,300px down. A `View all in Context` link on the heading opens `/context?project=<id>`.
  - *Revised 2026-09 (Tines/269):* each state group's heading carries `inherits from <workflow> / <state>` and `inherited by …`, both linking to `/workflows/<id>#state-<id>`. The base's own items are not shown here — they are state-scoped with no project, so the workflow page is the only surface that holds them.
- **Workflow detail** shows, per state, a small context indicator (count badge on the state's row/node); selecting a state reveals its non-issue-anchored items — `state`-only and `project ∧ state` (labeled with their project). Inline create defaults to that state. This is also where state removal warns about attached context and offers the forced delete with the item list.
  - *Revised 2026-09 (Tines/269):* each state row is anchored at `#state-<id>` (the deep link every inheritance line across the app uses; arriving on it opens that row) and its header carries `inherits from …` / `inherited by …`. An open row lists, below its own items, one read-only list per ancestor — root last, every row wearing the `via` chip — so the layers an issue in that state actually receives are visible in one place. The standard workflow's read-only table carries `inherited by …` in its spare column.
