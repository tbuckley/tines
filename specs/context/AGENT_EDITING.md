...
State inheritance is retired in Release B. The nullable pointer remains only
for historical schema and receipt inspection; matching and journal selection
are exact-state, and non-null workflow `inherits_from` inputs are rejected.
Use the reviewed retirement workflow for approved context copies. Project
descendants and a future journal-sharing contract are not implemented here.
...
**The journal is exact-state scoped.** `GET /api/v1/issues/:id/journal` and
the `tines journal` commands select the `journal` prompt at the current
project ∧ state scope (or the run-anchored state for a run key). There is no
root-ancestor resolution, automatic stitching, or read-only inherited journal
section. A future project-descendant or journal-sharing contract requires a
...
The section names the exact writable journal for the current stage:

```markdown
### Journal

Your journal for this project and stage is the exact current-state journal
(currently v7).

Appends land in this stage's journal even after you move the issue.
```

State names are rendered exactly as stored, including newlines; no inherited
scope labels or root aliases are introduced.
