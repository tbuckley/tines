...
- **Log.** Every run carries an append-only log, of which the last 256 KB lives in D1 as the live tail (`log_bytes_dropped` records how much has scrolled off it). Local daemons POST chunks as the harness emits output, each carrying a monotonic `seq` so a retried send is applied exactly once. For managed runs, each supervisor sweep polls the provider (Claude session events list; Gemini interaction `steps`) and appends a rendered summary of new events — tool calls, messages, status changes — since the last poll.
  ...
  Local `claude_code` runs invoke the harness with `--output-format stream-json` and the daemon renders that stream into the same `[agent]` / `[tool]` / `[session]` lines managed runs produce, so tool calls and shell commands are visible while the run is going and every surface reads prose rather than JSON. The unrendered stream is uploaded once at settle as the run's raw log.
...
Run keys cannot call the runner protocol, and are fenced off the control plane (`/runners*`, `/routing-rules*`, `/supervisor/settings`, `/issues/:id/resume`, key management, and — field-level — applying or removing a label a routing rule is scoped to) with 403s pointing at the proposal convention; runner tokens cannot call anything but the protocol endpoints. Key-management endpoints keep refusing all bearer-key auth as in phase one.
