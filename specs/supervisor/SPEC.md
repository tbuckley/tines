...
From the Gemini milestone (PLAN.md Milestone 3; verified against the live Interactions API and its SDK types, September 2026 — where these differ from the text above, they are the decision):

- **Tiers on Gemini.** The Antigravity agent runs the Flash family only (`gemini-3.8-flash`, 3.7, 3.6, 3.5 Flash, `gemini-3.5-flash-lite`); there is no Pro to point `smartest` at. Built-ins: `smartest` and `balanced` → `gemini-3.8-flash` (Antigravity's own default, released 2 September 2026), `cheapest` → `gemini-3.5-flash-lite`. The "Pro / Flash / Flash-Lite" illustration under Model tiers predates this.
- **Run tagging and reconciliation.** Interactions carry no title; the run id travels in `labels` (`tines_run_id`, `tines_runner_id`). The API lists environments but not interactions, so the provider-side half of launch reconciliation (cancel sessions tagged with unknown/ended runs) does not exist for Gemini: a crash between interaction create and the DB write leaves an interaction bounded by its own `max_total_tokens` and the environment's idle timeout. The DB-side half (fail session-less `launching` runs after 5 minutes) applies unchanged. The sweep deletes the environments of ended runs.
- **One environment per run.** Each launch creates a fresh `remote` environment inline in the interaction request — the sources are per issue anyway, and a standing environment would only accumulate state between unrelated runs. `repository` sources clone the default branch (the source has no branch field); the preamble names the branch to check out where a context item pins one.
- **The PAT at the proxy.** `github.com` gets `Authorization: Basic base64(<PAT>:x-oauth-basic)` — the documented form for the environment's clone of a private repository, which also authenticates the agent's pushes — and `api.github.com` gets `Bearer <PAT>`. Package registries and GitHub's download hosts are allowlisted without transforms so installs work; the allowlist is otherwise closed.
- **The CLI bundle** is fetched from the npm CDN (`https://cdn.jsdelivr.net/npm/tines@latest/dist/tines.cjs`, overridable with `TINES_CLI_BUNDLE_URL`) at launch and cached per isolate for ten minutes, not embedded in the worker: a merge publishes the CLI and deploys the worker together, so `latest` is what matches this deployment's prompts — the daemon's refresh reasons the same way — and the worker build stays independent of the CLI build. Until the first publish carrying `dist/tines.cjs` lands, Gemini launches fail with a message saying exactly that (a launch failure, never a strike).
- **Money on Gemini.** A minimal built-in pricing table ships now (`BUILTIN_MODEL_PRICING` in `@tines/shared`, list rates per model) so Gemini runs carry `cost_usd` with `cost_source: priced`; thinking tokens are recorded as `thought_tokens`, priced at the output rate, and stay outside `input + output` for caps. `max_run_cost_usd` — which Gemini cannot enforce natively — is enforced at poll time, so a run can overshoot its cap by up to one sweep interval; `max_run_tokens` → `agent_config.max_total_tokens` is the provider-enforced stop. Overrides, the warning treatment, and daily budgets remain Milestone 4.
- **Console link.** `provider_url` is AI Studio's logs page; no per-interaction deep link is documented.
- **No SDK.** The adapter speaks the REST API over `fetch`; `@google/genai` depends on google-auth-library, ws, and protobufjs, none of which belong in the worker bundle for four calls.
